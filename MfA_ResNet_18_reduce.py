...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, binary=True):
        ...
        self.left = nn.Sequential(
            MfA_Conv(in_channel, out_channel, high_L=3, high_S=1, low_L=3, low_S=3,
                     high_stride=stride, low_stride=stride, basic_stride=stride),
            nn.BatchNorm2d(out_channel*4),
            nn.ReLU(True),
            MfA_Conv(out_channel*4, out_channel, high_L=3, high_S=1, low_L=3, low_S=3),
            nn.BatchNorm2d(out_channel*4)
        )
...
class ResNet_cifar(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, M=1, method='MConv'):
        ...
        self.conv1 = MfA_Conv(3, 16, high_L=3, high_S=1, low_L=3, low_S=3) # FIXME: kernel_size=3?
